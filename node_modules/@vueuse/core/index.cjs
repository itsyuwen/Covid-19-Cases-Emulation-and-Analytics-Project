...
function useMediaQuery(query, options = {}) {
  const { window = defaultWindow } = options;
  const isSupported = Boolean(window && "matchMedia" in window && false);
  let mediaQuery;
...
function useDraggable(target, options = {}) {
  ...
  return __spreadProps$4(__spreadValues$c({}, shared.toRefs(position)), {
    position,
...
function useFetch(url, ...args) {
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    const context = { url: vueDemi.unref(url), options: __spreadValues$a(__spreadValues$a({}, defaultFetchOptions), fetchOptions), cancel: () => {
      isCanceled = true;
    } };
